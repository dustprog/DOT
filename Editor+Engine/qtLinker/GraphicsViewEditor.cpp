...
GraphcisViewEditor::GraphcisViewEditor(QGraphicsScene *scene, QWidget *parent)
...
QPoint GraphcisViewEditor::GetLocal(QPoint Window)
{
...
void GraphcisViewEditor::mousePressEvent(QMouseEvent *event)
{
...
void GraphcisViewEditor::mouseReleaseEvent(QMouseEvent *event)
{
    if (event->button() == Qt::LeftButton)
    {
        if (this->rect().contains(event->windowPos().toPoint()))
...
void GraphcisViewEditor::wheelEvent(QWheelEvent *event) {
    if (Control){
        b = false;
...
void GraphcisViewEditor::mouseMoveEvent(QMouseEvent *event)
{
    Last = Temp;
    Temp = event->localPos().toPoint();
    Temp = mapToScene(Temp).toPoint();
...
void GraphcisViewEditor::keyPressEvent(QKeyEvent *keyevent)
{
...
void GraphcisViewEditor::keyReleaseEvent(QKeyEvent *keyevent)
{
