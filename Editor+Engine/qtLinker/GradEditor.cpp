...
GradientEditor2DAdvanced::GradientEditor2DAdvanced(World *wo, Gradient *grad, QWidget *parent)
{
    ...
    newTB("Low", (void*)new std::string(std::to_string(grad->Low)), Link::INT);
    newTB("First Quartile", (void*)new std::string(std::to_string(grad->Q1)), Link::INT);
    newTB("Third Quartile", (void*)new std::string(std::to_string(grad->Q3)), Link::INT);
    newTB("Max", (void*)new std::string(std::to_string(grad->Max)), Link::INT);
    sbw = new SliderBoxWidget("Z: .2", 20);
    vLayout->addWidget(sbw);
    newTB("Current value", (void*)new std::string(std::to_string(200)), Link::INT);
