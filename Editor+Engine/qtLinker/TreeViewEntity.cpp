...
void TreeWindowEntity::setupModelData(TreeItem *parent, Advertisement *a)
{
    ...
    parents.at(0)->insertChildren(0, 2, 1);
    TreeItem * cost_root = parents.at(0)->child(0);
    TreeItem * inference_root = parents.at(0)->child(1);

    QVector<QVariant> columnData;

    cost_root->setData(0, QVariant("Cost"));


    inference_root->type = TreeType::ADVERTISEMENTADD;
    inference_root->Link = (void*)&a->Infer;
    inference_root->setData(0, QVariant("Inference"));

    //Advertisements
    //parents.at(0)->insertChildren(0, 2, 1);
    QVector<QVariant> addata; //Name
    cost_root->insertChildren(0, 2, 1);
    TreeItem *positive_costs = cost_root->child(0);
    TreeItem *negative_costs = cost_root->child(1);

    columnData.push_back("Positive");
    positive_costs->type = TreeType::COSTADD;
    positive_costs->Link = (void*)&a->positive_effects;
    positive_costs->setData(0, columnData[0]);
    columnData.clear();

    columnData.push_back("Negative");
    negative_costs->type = TreeType::COSTADD;
    negative_costs->Link = (void*)&a->negative_effects;
    negative_costs->setData(0, columnData[0]);
    columnData.clear();


    cost_root->child(0)->insertChildren(0, a->positive_effects.size(), 1); //Create an array of pos
    for (int i = 0; i < a->positive_effects.size(); i++)
    {
        addata.push_back(a->positive_effects.at(i).StringT.c_str());
        positive_costs->child(i)->setData(0, addata[0]);
        positive_costs->child(i)->type = TreeType::COST;
        positive_costs->child(i)->Link = (void*)&a->positive_effects.at(i);
        addata.clear();
    }

    negative_costs->insertChildren(0, a->negative_effects.size(), 1); //Create an array of neg
    for (int i = 0; i < a->negative_effects.size(); i++)
    {
        addata.push_back(a->negative_effects.at(i).StringT.c_str());
        negative_costs->child(i)->setData(0, addata[0]);
        negative_costs->child(i)->type = TreeType::COST;
        negative_costs->child(i)->Link = (void*)&a->negative_effects.at(i);
        addata.clear();
    }

    if (a->Infer != nullptr)
    {
        inference_root->insertChildren(0, 1, 1); //Create an array of neg
        addata.push_back(a->Infer->Name.c_str());
        inference_root->child(0)->setData(0, addata[0]);
        inference_root->child(0)->type = TreeType::ADVERTISEMENT;
        inference_root->child(0)->Link = (void*)a->Infer;
        addata.clear();
    }
}
